...
public class DocumentModificationExample
{
    ...
    public class DocumentV2
    {
        ...
        [ComputedHash(HashMethod.SHA2_256, nameof(Title), nameof(Content))] // Changed from SHA2_512 to SHA2_256
        public byte[]? ContentHash { get; private set; }
    }
    ...
    public class DocumentV3
    {
        ...
        [ComputedHash(HashMethod.SHA2_256, nameof(Title), nameof(Content), nameof(LastModified))] // Added LastModified
        public byte[]? ContentHash { get; private set; }
    }
...
public class StorageTypeExamples
{
    ...
    public static void DemonstrateStorageTypeInfo()
    {
        ...
        Console.WriteLine($"SHA2_256: {HashMethod.SHA2_256.GetHashSize()} bytes → {HashMethod.SHA2_256.GetRecommendedSqlType()}");
        Console.WriteLine($"SHA2_512: {HashMethod.SHA2_512.GetHashSize()} bytes → {HashMethod.SHA2_512.GetRecommendedSqlType()}");
        Console.WriteLine($"MD5: {HashMethod.MD5.GetHashSize()} bytes → {HashMethod.MD5.GetRecommendedSqlType()}");
        
        Console.WriteLine("\nSecurity Status:");
        Console.WriteLine($"SHA2_256: {(HashMethod.SHA2_256.IsCryptographicallySecure() ? "✅ Secure" : "❌ Insecure")}");
        Console.WriteLine($"SHA2_512: {(HashMethod.SHA2_512.IsCryptographicallySecure() ? "✅ Secure" : "❌ Insecure")}");
        Console.WriteLine($"MD5: {(HashMethod.MD5.IsCryptographicallySecure() ? "✅ Secure" : "❌ Insecure")}");
    }
} 